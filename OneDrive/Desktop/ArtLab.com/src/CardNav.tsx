...
const CardNav: React.FC<CardNavProps> = ({
  ...
  const MOBILE_TOP_BAR_HEIGHT = 60;

  const getTopBarHeight = () => {
    const isMobile = window.matchMedia('(max-width: 768px)').matches;
    return isMobile ? MOBILE_TOP_BAR_HEIGHT : DESKTOP_TOP_BAR_HEIGHT;
  };
  ...
  const createTimeline = () => {
    ...
    gsap.set(navEl, { height: getTopBarHeight(), overflow: 'hidden' });
  ...
  return (
    <div className={`card-nav-container ${className}`}>
      <nav
        ref={navRef}
        className={`card-nav card-nav-${theme} ${isExpanded ? 'open' : ''}`}
